...
8. Los puntos se suman al confirmar una orden. Si se usan como parte de pago se restan del saldo. Al cancelar una orden se intenta descontar del saldo los puntos que esa compra había generado; el saldo no baja de cero (si el usuario ya gastó puntos, no se puede “recuperar” más de lo que aún tiene). Los puntos canjeados en la compra no se devuelven.
...
13. Los clientes registrados pueden cancelar una orden dentro de las primeras 24 horas mediante PUT /usuarios/{id}/ordenes/{ordenId}/cancelar. Al cancelar, la orden pasa a estado CANCELADA, el stock se restaura, se restan del saldo los puntos generados por esa compra sin que el saldo quede por debajo de cero, y los puntos usados como descuento no se reintegran. Así se limita el abuso de canjear puntos y cancelar dejando saldos negativos o inconsistencias.
